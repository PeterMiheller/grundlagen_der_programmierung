...
def count_palindromes(underage, filename):
    with open(filename, 'r') as file:
        ...
        lines = list(map(lambda x: x.strip().split(','), content))
        pers = list(filter(lambda x: x[0].lower() == x[0][::-1].lower(), lines))
        if underage:
            minderjh = list(filter(lambda x: int(x[1]) <= 18, pers))
            return len(minderjh)
        else:
            return len(pers)


print('-underage + palindrom: ', count_palindromes(True, 'people.txt'))
print('-all names that are palindromes: ', count_palindromes(False, 'people.txt'))
